...
class sessions:
    ...
    SessionNotRecorded = RecordNotCreatedError
    MessageNotRecorded = RecordNotCreatedError
    PromptNotRecorded = RecordNotCreatedError
    SessionAttributeNotUpdated = RecordUpdateError
    DBRecordError = RecordNotCreatedError
    DBAttributeError = RecordUpdateError
    ...
    def create_session(self, user_id, oai_api_key, bot_id=None, bot_config_dict=None, overwritten_model=None):
        ...
        if bot_id != None:
            ...
            raise BadRequestError("Bad request: missing both Assistant ID and Assistant config")
        ...
        if bot_id != None:
            # increment user's and bot's sessions_started field (non-critical, swallow errors)
            try:
    ...
    def get_session_response(self, session_id, oai_api_key, user_message=None):
        ...
        if current_session == None:
            raise BadRequestError("Bad request: session not found")
    ...
    def end_session(self, session_id, end_status=SessionStatus.COMPLETED.value):
        ...
        if end_status not in [e.value for e in self.SessionStatus]:
            raise BadRequestError("Invalid end_status value")
        ...
        if end_status == self.SessionStatus.COMPLETED.value:
            # increment user's and bot's sessions_ended field (non-critical, swallow errors)
            try:
    ...
    def rate_session(self, session_id, user_liked=UserLiked.LIKED.value):
        ...
        # Rating is non-critical, don't raise on failure
        self.db.update_document_fields(collection_name="sessions", document_id=session_id, updates=user_liked_update)
    ...
    def _create_session(self, user_id, bot_id=None, bot_config_dict=None, overwritten_model=None):
        ...
        if bot_id != None:
            ...
            try:
                ...
                raise BadRequestError("Bad Request: Assistant not found")
            except bot.BotIncomplete as e:
                raise BadRequestError("Bad Request: Assistant is not fully configured")
            except Exception as e:
                raise BadRequestError("Bad Request: Unknown error")
        ...
        if bot_config_dict != None:
            ...
            if bot_config_validated == False:
                raise BadRequestError("Bad Request: Assistant Config Missing Required Fields")
            bot_dict = bot_config_dict

        try:
            session_type = bot.get_session_type(bot_dict['session_type'])
        except ValueError as e:
            raise BadRequestError("Bad Request: Assisant configured with unknown session type")
        ...
        if not session_id:
            raise RecordNotCreatedError("DB Record Error: session not created")
    ...
    def _update_session_messages(self, session_id, message_response_dict):
        ...
        if record_new_msg == None:
            raise RecordUpdateError("DB Attribute Error: session messages not updated")
    ...
    def _record_session_message(self, session_id, message, is_user, record_message=True):
        if record_message:
            ...
            msg_dict = {
                'message':gu.encrypt_user_message(user_hash=self.user_hash, user_message=message.replace(self.openai_restart_sequence,"").replace(self.openai_stop_sequence,"")),
                'role':msg_role,
                'created_date':gu.get_current_time()
            }
            ...
            if message_id == None:
                raise RecordNotCreatedError("DB Record Error: message not recorded")

        # increment message count fields (non-critical, swallow errors)
        try:
